...
def get_connection_info(display_name="Client"):
...
async def run_client(ws_url, room_id, peer_id, credentials):
    pc = RTCPeerConnection(RTCConfiguration(
        iceServers=[RTCIceServer(urls=["stun:stun.rtc.yandex.net:3478"])]
    ))
    
    dc = pc.createDataChannel("olcrtc", ordered=True)
    ...
    async with websockets.connect(ws_url) as ws:
        ...
        try:
            print(f"[*] Waiting for DataChannel to open...")
            await asyncio.wait_for(dc_open.wait(), timeout=15.0)
            
            print(f"\n[*] DataChannel ready! Sending test messages...\n")
            
            messages = [
                "Hello from OlcRTC Client!",
                "Testing DataChannel communication",
                "X" * 1000,
                "Final test message"
            ]
            
            for i, msg in enumerate(messages, 1):
                print(f"[>] Sending message {i}/{len(messages)}: {msg[:50]}{'...' if len(msg) > 50 else ''}")
                try:
                    dc.send(msg)
            ...
            print(f"    Received: {len(received_messages)} responses")
...
async def main():
    print(f"{'='*60}")
    print(f"  OlcRTC Client - Yandex Telemost DataChannel")
    print(f"{'='*60}\n")
    ...
    await run_client(ws_url, room_id, peer_id, credentials)
    
    print(f"\n{'='*60}")
    print(f"  :P Client finished")
    print(f"{'='*60}")
